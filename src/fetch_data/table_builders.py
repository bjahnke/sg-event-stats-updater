...
def get_performer_events(performer_name: str, performers_df: pd.DataFrame, performers_events: pd.DataFrame) -> pd.Series:
    """
    ...
    :return:
    """
    performer_id = performers_df.loc[performers_df['name'] == performer_name]['id'].values[0]
    return performers_events.loc[performers_events['performer_id'] == performer_id, 'event_id']
...
def select_events_by_event_id(events_df: pd.DataFrame, event_ids: pd.Series) -> pd.DataFrame:
    """
...
def build_stats_df(events_df: pd.DataFrame) -> pd.DataFrame:
    ...
    stats_df = pd.concat([stats_df.drop(['stats'], axis=1), stats_df['stats'].apply(pd.Series)], axis=1)
    # keep only event_id, average_price, lowest_price, highest_price, listing_count, visible_listing_count, median_price
    stats_df = stats_df[['event_id', 'average_price', 'lowest_price', 'highest_price', 'median_price', 'listing_count', 'visible_listing_count']]
...
def build_performer_events_df(events: pd.DataFrame) -> pd.DataFrame:
    ...
    performers_events['performer_id'] = performers_events['performers'].apply(lambda x: x['id'])
...
def build_df_from_series_of_dicts(series_of_dicts: pd.Series, ) -> pd.DataFrame:
    """
...
# the table builder functions above
    """
    def __init__(self, events: pd.DataFrame, performers_df: pd.DataFrame, venue_df: pd.DataFrame):
        ...
        self.events = self.events.drop(columns=['performers', 'links', 'datetime_tbd', 'time_tbd', 'stats', 'taxonomies', 'date_tbd', 'description', 'event_promotion', 'announcements', 'conditional', 'enddatetime_utc', 'is_visible_override', 'tdc_pvo_id', 'tdc_pv_id', 'themes', 'domain_information', 'venue'])
        ...
        self.performers_df = self.performers_df.drop(columns=['image', 'images', 'divisions', 'links', 'primary', 'stats', 'taxonomies', 'image_attribution', 'colors', 'location', 'image_rights_message', 'themes', 'domain_information'])
    ...
    def from_api(cls, client: Scalpyr):
        ...
        events = pd.DataFrame.from_dict(
            client.get_events({'per_page': "1000", 'type': 'concert', 'sort': 'score.desc'})['events']
        )
    ...
    def get_performer_events(self, performer_name: str) -> pd.Series:
        return get_performer_events(performer_name, self.performers_df, self.performer_events_df)
    ...
    def get_performer_events_df(self, performer_name: str) -> pd.DataFrame:
        return get_performer_events_df(performer_name, self.performers_df, self.events, self.performer_events_df)
    ...
    def push_to_db(self, engine):
        ...
        new_performer_events_df.to_sql('performer_event_venue', engine, if_exists='append', index=False)
